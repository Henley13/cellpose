...
def main():
    ...
    parser.add_argument('--check_mkl', action='store_true',
    ...
    parser.add_argument('--resample', action='store_true',
                        help="run dynamics on full image (slower for images with large diameters)")
    ...
    parser.add_argument('--save_tif', action='store_true',
    ...
    if len(args.dir) == 0:
        ...
        if not args.train and not args.train_size:
            tic = time.time()
            if not (
                    args.pretrained_model == 'cyto' or args.pretrained_model == 'nuclei'):
            ...
            image_names = io.get_image_files(args.dir, args.mask_filter,
                                             imf=imf)
            ...
            for image_name in tqdm(image_names):
                ...
                if args.save_png or args.save_tif:
                    io.save_masks(image, masks, flows, image_name,
                                  png=args.save_png, tif=args.save_tif)
            print('>>>> completed in %0.3f sec' % (time.time() - tic))
        else:
            if args.pretrained_model == 'cyto' or args.pretrained_model == 'nuclei':
                torch_str = ['torch', '']
                cpmodel_path = os.fspath(model_dir.joinpath(
                    '%s%s_0' % (args.pretrained_model, torch_str[args.mxnet])))
            ...
            if rescale and args.train:
                print(
                    '>>>> during training rescaling images to fixed diameter of %0.1f pixels' % args.diameter)
            ...
            if args.train:
                cpmodel_path = model.train(images, labels,
            ...
            if args.train_size:
                ...
                if test_images is not None:
                    ...
                    ccs = np.corrcoef(diams_style, np.array(
                        [utils.diameters(lbl)[0] for lbl in tlabels]))[0, 1]
                    cc = np.corrcoef(predicted_diams, np.array(
                        [utils.diameters(lbl)[0] for lbl in tlabels]))[0, 1]
                    print(
                        'style test correlation: %0.4f; final test correlation: %0.4f' % (
                        ccs, cc))
                    np.save(os.path.join(args.test_dir,
                                         '%s_predicted_diams.npy' %
